...
- Ejecución aislada de funciones: carga dinámica desde `src/matthews_order_backend/functions/*`, con soporte async/sync y timeouts.
...
--------------------
Define un `.env` siguiendo `example.env`. Las claves disponibles están en `src/matthews_order_backend/settings.py`:
...
```json
{
  "test-slow-echo": {
    ...
    "function": "testing.slow_echo",// Ruta dentro de matthews_order_backend.functions
...
- Construye la imagen:
```bash
docker build -t matthews_order_backend .
```
...
Bot de Discord
--------------
- Activo con `poetry run discord` o el servicio `matthews_order_backend_discord`.
...
----------------------
- `src/matthews_order_backend/app.py`: punto de entrada. Crea FastAPI con lifespan y enruta `/healthz` y `/order`. El modo `discord` inicializa el cliente `OrderDiscordClient`.
- `src/matthews_order_backend/settings.py`: configuración con `pydantic-settings`, lectura opcional de `.env` (cuando `IS_DOCKER_CONTAINER` es falso).
- `src/matthews_order_backend/app_utils.py`: cachea settings y `ConfigRepository`, limpia `_` campos sensibles al mostrar config, ejecuta funciones sync/async, y expone `reset_runtime_state` para tests.
- Modelos (`src/matthews_order_backend/models/*`):
  - `actions.py`: `ActionConfig` (timeout, passkey, entorno, función), `OrderRequest/Response`, `ConfigRepository` (recarga `api_config.json` cuando cambia) y `FunctionRegistry` (importa dinámicamente desde `matthews_order_backend.functions`).
  - `ai/*`: `ActionSelectionRequest/Result` (parsea JSON del LLM con `utils.json`), `TalkRequest/Result`.
- Endpoints REST (`src/matthews_order_backend/endpoints/rest/*`):
...
- Bot de Discord (`src/matthews_order_backend/endpoints/discord/order_event.py`): flujo descrito arriba; usa `FUNCTION_OUTPUT_MESSAGE_MODES` para modular los mensajes.
- Clientes de IA (`src/matthews_order_backend/ai/*`):
...
- Funciones (`src/matthews_order_backend/functions/*`):
  ...
  - `webscraping/padel/padel_checker_vigo_twelve.py`: pipeline WIP con Autoweb + OpenRouter.
  - Cada módulo puede fijar `DEFAULT_FUNCTION_OUTPUT_MESSAGE_MODE` (`ASSISTANT` o `EXECUTION`) para que el bot decida cómo responder.
- Utilidades (`src/matthews_order_backend/utils/*`): limpieza de ANSI en logs de comandos (`text.py`), parseo robusto de JSON devuelto por LLMs (`json.py`).
- Logging (`src/matthews_order_backend/logger/*`): configuración dictConfig y helper `get_logger`.
...
- Para añadir acciones nuevas: crea un módulo en `src/matthews_order_backend/functions/...` con `run(*, environment, payload)` y regístralo en `api_config.json` con su entorno y passkey si aplica.
